...
router.get("/", (req, resp) => {
  Orders.findAll({include: [{model: OrderProduct, include: [Products]}, Users]}).then(orders => {
    resp.send(orders);
  });
});

router.get("/:id", (req, resp) => {
  Orders.findByPk(req.params.id, {include: [{model: OrderProduct, include: [Products]}, Users]}).then(order => {
...
router.post("/", (req, resp) => {
  console.log(req)
  Orders.create(req.body, {include: [OrderProduct]}).then(order => {
